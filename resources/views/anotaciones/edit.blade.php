            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('anotaciones.update', $anotacion) }}">
                        ...
                        <div class="mb-4">
                            <label class="block font-medium text-sm text-gray-700 dark:text-gray-300" for="puesto">
                                Puesto en la clase (1 = mejor, 30 = peor)
                            </label>
                            <input id="puesto" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm w-full mt-1" name="puesto" type="number" min="1" max="30" value="{{ old('puesto', $anotacion->puesto) }}">
                            <p class="text-xs text-gray-500 mt-1">Opcional. Dejar vacío si no se ha asignado puesto.</p>
                            @error('puesto')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
