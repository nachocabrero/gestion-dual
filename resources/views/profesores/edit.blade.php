                    ...
                    <form method="POST" action="{{ route('profesores.update', $profesor) }}">
                        ...
                        <div class="mb-4 flex gap-4">
                            <div class="flex-1">
                                ...
                                @php
                                    $tutorGrupoId = $profesor->user->tutorias()->first()->id ?? null;
                                @endphp
                        ...
                        <div class="mb-4">
                            ...
                            <div class="max-h-40 overflow-y-auto border rounded p-2 text-gray-800 dark:text-gray-200">
                                ...
                                <label class="flex items-center text-sm">
                                    <input type="checkbox" name="asignatura_ids[]" value="{{ $asig->id }}" {{ in_array($asig->id, old('asignatura_ids', $profesor->asignaturas->pluck('id')->toArray())) ? 'checked' : '' }} class="mr-2">
                                    {{ $asig->nombre }}
                                </label>
                                @endforeach
                                @endforeach
                        ...
                        <div class="mb-4">
                            ...
                            <div class="max-h-40 overflow-y-auto border rounded p-2 text-gray-800 dark:text-gray-200">
                                ...
                                @foreach($linea->grupos as $grupo)
                                <label class="flex items-center text-sm">
                                    <input type="checkbox" name="grupo_ids[]" value="{{ $grupo->id }}" {{ in_array($grupo->id, old('grupo_ids', $profesor->gruposImpartidos->pluck('id')->toArray())) ? 'checked' : '' }} class="mr-2">
                                    {{ $grupo->nombre }}
                                </label>
                                @endforeach
                                @endforeach
