...
<div class="max-w-3xl mx-auto">
    <h1 class="text-2xl font-bold text-white mb-6">Nuevo Convenio / Acuerdo Formativo</h1>
        ...
        <div class="bg-gray-800 rounded-lg p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="tutor_docente_id" required class="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white text-sm">
                        ...
                        @foreach($profesores as $profesor)
                            <option value="{{ $profesor->id }}" {{ old('tutor_docente_id') == $profesor->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="tutor_laboral_id" required class="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white text-sm">
                        ...
                        @foreach($tutores as $tutor)
                            <option value="{{ $tutor->id }}" {{ old('tutor_laboral_id') == $tutor->id ? 'selected' : '' }}>
                                {{ $tutor->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="date" name="fecha_inicio" required value="{{ old('fecha_inicio') }}" class="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white text-sm">
                </div>

                <div>
                    <label class="block text-gray-400 text-sm mb-1">Fecha Fin *</label>
                    <input type="date" name="fecha_fin" required value="{{ old('fecha_fin') }}" class="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white text-sm">
                </div>

                <div>
                    <label class="block text-gray-400 text-sm mb-1">Número de horas *</label>
                    <input type="number" name="numero_horas" required min="1" value="{{ old('numero_horas', 500) }}" class="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white text-sm">
                </div>
                ...
                <div>
                    ...
                    <select name="estado" class="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white text-sm">
                        <option value="no_firmado" {{ old('estado') == 'no_firmado' ? 'selected' : '' }}>No firmado</option>
                ...
                <div>
                    ...
                    <input type="date" name="fecha_firma" value="{{ old('fecha_firma') }}" class="w-full bg-gray-700 border border-gray-600 rounded px-3 py-2 text-white text-sm">
                </div>
            </div>
        </div>
